...
One Works Web 可以作为普通 Web UI 使用，也可以把 client 单独构建成静态 PWA，部署到 GitHub Pages、对象存储、CDN 或其他静态站点，再让用户在首次打开时填写后端服务地址。

## 适用场景

- 同源部署：server 直接托管 Web UI，用户访问 `/ui`。这是最省心的模式，前后端同源，不需要额外处理浏览器跨域和 HTTPS 混合内容。
- 独立部署：client 部署到静态站点，后端单独运行。用户首次打开页面时输入后端地址，例如 `https://<machine>.<tailnet>.ts.net` 或 `https://oneworks-api.example.com`。
...
PWA 连接页里展示的参考脚本适合本机浏览器、Tailscale Serve、Cloudflare Tunnel、ngrok 或反向代理这类“本机 server + 外层 HTTPS 入口”的场景：

```bash
npx oneworks server --allow-cors
```

`bootstrap server` 会按需懒安装并转发到 `@oneworks/server`。复制终端里显示的本机地址用于同一台电脑访问；如果外层还有 HTTPS tunnel 或反向代理，PWA 里应填写外层提供的 `https://...` 地址，而不是终端里的本机地址。

如果需要同一个局域网内的其他设备直接访问，可以绑定到所有网卡。这个模式会暴露给当前网络里的其他设备，应确认网络可信或显式配置登录账号：

```bash
npx oneworks server --host 0.0.0.0 --port 8787 --allow-cors
```

公网 tunnel、反向代理或任何不只限于私人网络的入口，都应显式开启并配置登录账号：
...
Cloudflare Tunnel 暴露的是公网入口。除了 One Works 自身的 `webAuth`，建议再使用 Cloudflare Access、IP allowlist 或其他身份校验。
